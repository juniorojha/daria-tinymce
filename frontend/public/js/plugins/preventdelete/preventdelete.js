(function() {
	...
	function PreventDelete() {
		...
		// Funções substituindo Jquery métodos
function getParents(elSelector, matchSelector /* optional */) {
	if (!elSelector) return null
	// Aceita node ou seletor como elemento base
	else if (typeof elSelector === 'string') const nodeList = document.querySelectorAll(elSelector)
	else const nodeList = elSelector;

	// Checar se é node element (node ou nodeList)
	if (!nodeList instanceof Element) return false;
	console.log('nodeList', nodeList);

	// Se el é uma NodeList
	if (NodeList.prototype.isPrototypeOf(nodeList) && nodeList.length > 1) {
		let parents = [];
		nodeList.forEach(function(element) {
			while ((element = element.parentElement.closest(matchSelector)))
				if (parents.indexOf(element) == -1) parents.push(element);
		});
		return parents;
	}

	// Aceita node ou seletor como elemento base
	if (typeof elSelector === 'string') const node = document.querySelector(elSelector)
	else const node = elSelector;
	let nodeParents = [];
	while ((node = node.parentElement.closest(matchSelector)))
		nodeParents.push(node);
	return nodeParents;
}

function findChildren(elSelector, matchSelector /* optional */) {
	...
	// Aceita node ou seletor como elemento base
			...
			if (NodeList.prototype.isPrototypeOf(nodeList) && nodeList.length > 1) {
				...
				for(let i=0; i<nodeList.length; i++) {
					let element = nodeList[i].querySelectorAll(matchSelector);
					...
					foundElements = foundElements.concat(element);
				}
			}

			// Aceita node ou seletor como elemento base
			if (typeof elSelector === 'string') const nodeFound = document.querySelector(elSelector).querySelectorAll(matchSelector);
			else const nodeFound = elSelector.querySelectorAll(matchSelector);
			return nodeFound;
		}
		...
		this.preventdelete_class = 'laudite'

		this.nextElement = function (elem) {
			// Aceita node ou seletor como elemento base
			if (typeof elem === 'string') const element = document.querySelector(elem)
			else const element = elem;
			let nextSibling = element.nextElementSibling;
			while(nextSibling.length == 0){
        element = element.parentElement
				if (element.getAttribute('id') == self.root_id) return false;
		...
		this.prevElement = function(elem) {
			...
			let prevSibiling = element.previousElementSibling;
			while(prevSibiling.length == 0){
                element = element.parentElement
                if(element.getAttribute('id') == self.root_id)
                	return false

                prevSibiling = element.previousElementSibling
			}
		...
		this.check = function (node) {
			// Aceita node ou seletor como elemento base
			if (typeof node === 'string') const elem = document.querySelector(node);
			else const elem = node;
			return elem.classList.contains(self.preventdelete_class);
		}
		...
		this.checkParents = function(node) {
			...
			return getParents(node,`.${self.preventdelete_class}`).length > 0
		}
		...
		this.checkChildren = function(node) {
			...
			return findChildren(node,`.${self.preventdelete_class}`).length > 0
		}
